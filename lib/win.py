...
"""
win32 backend to send input to windows

Thanks to Noctis Skytower :)
see https://stackoverflow.com/a/11910555/10330869
...
def _Input(structure):
    if isinstance(structure, _MOUSEINPUT):
        return _INPUT(INPUT_MOUSE, _INPUTunion(mi=structure))
    if isinstance(structure, _KEYBDINPUT):
        return _INPUT(INPUT_KEYBOARD, _INPUTunion(ki=structure))
    if isinstance(structure, _HARDWAREINPUT):
        return _INPUT(INPUT_HARDWARE, _INPUTunion(hi=structure))
...
def _MouseInput(flags, x, y, data):
    return _MOUSEINPUT(x, y, data, flags, 0, None)
...
def _KeybdInput(code, flags):
    return _KEYBDINPUT(code, code, flags, 0, None)


def _HardwareInput(message, parameter):
    return _HARDWAREINPUT(message & 0xFFFFFFFF,
                          parameter & 0xFFFF,
                          parameter >> 16 & 0xFFFF)


def Mouse(flags, x=0, y=0, data=0):
    return _Input(_MouseInput(flags, x, y, data))


def Keyboard(code, flags=0):
    return _Input(_KeybdInput(code, flags))


def Hardware(message, parameter=0):
    return _Input(_HardwareInput(message, parameter))
...
def stream_chars(string):
    ...
    for character in string.replace('\r\n', '\r').replace('\n', '\r'):
        ...
        if character in ORDER:
            ...
            raise ValueError(f'Char {character} is not supported!')
        yield Keyboard(code)
...
def _make_hndl(hndl: int) -> W_HNDL:
    return None if hndl == 0 else hndl


def find_window(name: str) -> W_HNDL:
    return _make_hndl(win32gui.FindWindow(None, name))


def get_active_window() -> W_HNDL:
    return _make_hndl(win32gui.GetForegroundWindow())


def is_window_focused(hndl: W_HNDL) -> bool:
    return False if hndl is None else hndl == get_active_window()
